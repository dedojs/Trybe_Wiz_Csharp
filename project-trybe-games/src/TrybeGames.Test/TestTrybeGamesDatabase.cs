...
public class TestTrybeGamesDatabase
{
    ...
    {
        // Arrange
        Player player = new Player { Id = playerIdEntry };

        // Act
        var gamesPlayedBy = databaseEntry.GetGamesPlayedBy(player);

        // Assert
        gamesPlayedBy.Should().BeEquivalentTo(expected);
    }
    ...
    public void TestGetGamesOwnedBy(TrybeGamesDatabase databaseEntry, int playerIdEntry, List<Game> expected)
    {
        ...
        // Act
        var gamesOwnedBy = databaseEntry.GetGamesOwnedBy(player);
        // Assert
        gamesOwnedBy.Should().BeEquivalentTo(expected);
    }
    ...
    public void TestGetGamesDevelopedBy(TrybeGamesDatabase databaseEntry, int gameStudioIdEntry, List<Game> expected)
    {
        ...
        // Arrange
        GameStudio gameStudio = new GameStudio { Id = gameStudioIdEntry };

        // Act
        var gamesDevelopedBy = databaseEntry.GetGamesDevelopedBy(gameStudio);

        // Assert
        gamesDevelopedBy.Should().BeEquivalentTo(expected);
    }
